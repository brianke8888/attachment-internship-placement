...
2. Create a new database (e.g. `attachment-system`).
3. Open the PlanetScale shell and run the table creation statements from `backend/schema.sql` (copy-paste the `CREATE TABLE` blocks).
4. Note your connection details: host, username, password, database name.
...
> **Note:** Render's free tier spins down after inactivity. The first request after idle may take 30-60 seconds.
